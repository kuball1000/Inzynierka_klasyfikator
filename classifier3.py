...
ENSEMBLE_K_VALUES = [3, 4, 5, 6, 10] # Ensemble K values - optimized
...
def predict_single_k(
    sims: np.ndarray,
    idx: IntentIndex,
    k: int
) -> Tuple[str, float]:
    """Pomocnicza funkcja do obliczania wyniku dla pojedynczego K."""
...
def predict_endpoint(
    ...
    if k_values is None:
        k_values = ENSEMBLE_K_VALUES
        
    q_emb = embed_queries([user_text])
    
    sims = cosine_similarity(q_emb, idx.embeddings)[0]
    
    ensemble_votes = [] # List[(endpoint, score)]
    
    # Debug info
    k_results = {}
    
    for k in k_values:
        ep, score = predict_single_k(sims, idx, k)
        ensemble_votes.append(ep)
        k_results[f"k{k}"] = {"endpoint": ep, "score": round(score, 4)}
        
    # Głosowanie większościowe między modelami K
    # Jeśli remis, wygrywa ten który był wskazany przez "preferowane K" (np. 5) albo po prostu ten z najwyższym średnim score.
    # Użyjmy Countera
    ...
    for k, res in k_results.items():
        ...
        if ep not in candidates:
            candidates[ep] = {"votes": 0, "total_score": 0.0, "max_score": 0.0}
        candidates[ep]["votes"] += 1
        candidates[ep]["total_score"] += sc
        candidates[ep]["max_score"] = max(candidates[ep]["max_score"], sc)
        
    # Wybór najlepszego kandydata
    # Sortujemy:
    # 1. Liczba głosów (malejąco)
    # 2. Max score (malejąco) - jako tie breaker
    
    sorted_candidates = sorted(
        candidates.items(), 
        key=lambda item: (item[1]["votes"], item[1]["max_score"]), 
        reverse=True
    )
    
    best_ep = sorted_candidates[0][0]
    best_candidate_stats = sorted_candidates[0][1]
    
    # Final score: użyjmy max score z głosowań (to jest score od najbliższego sąsiada w ramach zwycięskich K)
    final_score = best_candidate_stats["max_score"]
    
    # Fallback thresholds
    if sim_threshold is None: sim_threshold = SIM_THRESHOLD
    if maybe_threshold is None: maybe_threshold = MAYBE_THRESHOLD

    method_str = f"ensemble_votes_k{''.join(str(k) for k in k_values)}"
    ...
    if final_score >= sim_threshold:
        return {
            ...
            "method": method_str,
            "debug": {"ensemble": k_results, "votes": {k: v["votes"] for k,v in candidates.items()}}
        }
