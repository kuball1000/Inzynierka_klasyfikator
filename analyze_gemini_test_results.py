...
def analyze_results(input_file, output_file):
    ...
    for item in data:
        true_ep = item.get('true')
        pred_ep = item.get('pred')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Analyze samples_debug_gemini.json and export stats to TSV.")
    parser.add_argument("--input", default="samples_debug_gemini.json", help="Input JSON file path")
    parser.add_argument("--output", default="analysis_gemini_test_results.tsv", help="Output TSV file path")
